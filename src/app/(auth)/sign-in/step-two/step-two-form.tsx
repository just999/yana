...
const StepTwoForm = () => {
  ...
  //     updateUserDetails({ [name]: fieldValue });
  //     // Clear the error for this specific field
  //     setClearedErrors((prev) => new Set(prev).add(name));
  //   }

  //   // Clear the specific field error from Jotai - Now with proper types
  //   if (validationErrors?.fieldErrors?.[fieldName as string]) {
  ...
  const handleInputChange = (
    ...
    if (typeof e === 'boolean' && fieldName) {
      // Handle boolean values (for custom checkbox components)
    ...
    // Clear the specific field error from validation
  ...
  // Specific handler for checkbox components (like shadcn/ui Checkbox)
  const handleCheckboxChange = (
    ...
    if (typeof checked === 'string') {
      ...
      booleanValue = checked === true; // handles both boolean true and 'indeterminate'
    }
  ...
  const onDrop = useCallback(
    ...
    [setFileAvatar, updateUserDetails]
  );
  ...
  useEffect(() => {
    return () => {
      ...
      // Also clean up any avatar preview in context when unmounting
